...
import { setUserOnline, setUserOffline, getOnlineUsers, cleanupOfflineUsers } from './userStatus.js';
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", async () => {
    ...
    if (userId) {
      // Marcar usuario como offline en Supabase
      await setUserOffline(userId);
    }
    
    // Obtener usuarios online actualizados desde Supabase y emitir
    const onlineUsers = await getOnlineUsers();
    console.log("Updated online users from Supabase:", onlineUsers);
    io.emit("getOnlineUsers", onlineUsers);
  });
});

// Limpiar usuarios offline cada 2 minutos
setInterval(async () => {
  await cleanupOfflineUsers();
  // Emitir usuarios online actualizados después de la limpieza
  const onlineUsers = await getOnlineUsers();
  io.emit("getOnlineUsers", onlineUsers);
}, 2 * 60 * 1000);

// Función para obtener usuarios online (para usar desde otros módulos)
export const getCurrentOnlineUsers = async () => {
